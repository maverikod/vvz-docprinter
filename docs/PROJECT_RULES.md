...
| Key | Value (this repo) | Description |
|-----|-------------------|-------------|
| `PROJECT_SLUG` | `docprinter` | Short ASCII id for paths and package naming. |
| `PRIMARY_LANGUAGE` | `Python` | Main implementation language (`Python`, `TypeScript`, …). |
| `PACKAGE_ROOT` | `docprinter/` | Root directory for production code (**no `src/`** unless profile explicitly allows). |
...
| `USE_CODE_MAP` | `no` | If `yes`, run the project’s **code map / index** tool after logical code-structure changes (this template calls it **`code_mapper`** / `code_analysis/` — rename in profile if your stack differs). |
...
| **LAYOUT-08** | **Runtime ephemeral data** under **`runtime/`** at repository root: server-managed working files that are **not** source, **not** logs, **not** config. Tracked placeholders only (`.gitkeep`); contents ignored by git. |
| **LAYOUT-09** | **`runtime/output/`** — final command artifacts addressable by id (e.g. `<uuid4>.docx`). Files are subject to **TTL eviction** by a background sweeper (interval and TTL from server config). Consumers **must** retrieve the file before TTL expires; the server makes no durability guarantees. |
| **LAYOUT-10** | **`runtime/work/`** — per-request working area for unpacking inputs and intermediate files. Each request **must** use an isolated subdirectory (e.g. `runtime/work/<uuid4>/`) and remove it on completion (success or error). Stale subdirectories are also subject to the runtime sweeper. |
...
<repo>/
  ...
  docker/              # Dockerfile and root-invoked helper scripts (this repo)
  logs/
  config/              # sample server JSON (runtime; this repo uses config/ not configs/)
  docs/
    ai_reports/
  projectid            # if CR-003 enabled for this project
  <VENV_DIR>/          # local, usually not committed
  runtime/             # ephemeral, gitignored (LAYOUT-08)
    output/            # rendered artifacts, TTL-evicted (LAYOUT-09)
    work/              # per-request scratch dirs (LAYOUT-10)
```
...
| Key | Value |
|-----|-------|
| `PROJECT_SLUG` | `docprinter` |
| `PRIMARY_LANGUAGE` | `Python` |
| `PACKAGE_ROOT` | `docprinter/` |
